...
class YoutubeAutomationTasks():
    ...
    def manage_youtube_video_creation(self, agent, video_topic, video_details):
        return Task(
            ...
            """),
            agent=agent,
            output_file="output/YouTube_Video_Creation_Report_english.txt",
    ...
    def create_youtube_video_title(self, agent, video_topic, video_details):
        return Task(
            description=dedent(f"""Create 10 potential titles for a given YouTube video topic and description. 
                ...
                Make sure translate the topics to tamil
            ...
            expected_output=dedent(f"""
                ...
                - in tamil -> CrewAI பயிற்சி: சமீபத்திய CrewAI அம்சங்களை எவ்வாறு பயன்படுத்துவது என்பதை அறிக
                - CrewAI Tutorial: Complete Crash Course for Beginners
                - in tamil -> CrewAI பயிற்சி: முழுமையான சுருக்கப் பாடம்
                ...                
                """),
        )

    def create_youtube_video_description(self, agent, video_topic, video_details, language):
        return Task(
            description=dedent(f"""Create a description for a given YouTube video topic and description in {language}.     
                Video Topic: {video_topic}
                Video Details: {video_details}
                """),
            agent=agent,
